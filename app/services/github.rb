class Github
  def self.get_basic_user_details(oauth_data)
    ...
    return {success: true, user_details: user_details, access_token: "#{access_data[:access_token]}"}
  end
  ...
  def self.get_access_token(code)
    ...
    return {success: false, error: json_response["error_description"]}
  end

  def self.get_user_details(access_data)
    url = 'https://api.github.com/user'
    response = RestClient::Request.execute(method: :get, url: url, headers: { Authorization: "token #{access_data[:access_token]}"})
